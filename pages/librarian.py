...
def connect_to_vectorstore():
    #TODO make qdrant run on cloud?
    client = QdrantClient(host="localhost", port=6333, path=r"""C:\Users\andre\QdrantStorage""")
    try:
...
with st.sidebar:
    ...
    #openai_api_key = st.text_input(label=st.secrets["openai_key"], placeholder="Paste your OpenAI API key here", type="password")
    #notion_api_key = st.text_input(label=st.secrets["openai_key"], placeholder="Paste your Notion API key here", type="password")

    print("\n")
    print("caching headers:")
    notion_headers = cache_headers(notion_api_key)
    print("response: ", notion_headers)
